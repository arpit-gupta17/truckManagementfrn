...
const Dashboard = () => {
  ...
  const { id } = useParams();
  const { companyId } = useParams(); // company id from route
    ...
    const fetchDrivers = async () => {
      try {
        const res = await axios.get(
          'http://localhost:5000/api/driver/getDrivers',
          ...
          <div className="bg-white rounded shadow-sm border border-gray-200 mb-5 overflow-hidden">
            ...
            <div className="p-3.5 border-b border-gray-200 flex items-center flex-wrap gap-2">
              {[
                ...
                <button
                  key={label}
                  className="px-3 py-1.5 rounded border border-gray-300 bg-white text-gray-800 text-sm font-medium flex items-center hover:bg-gray-50"
                >
                ...
                <tbody className="bg-white divide-y divide-slate-300 text-slate-700">
                  {drivers.map((driver) => (
                    <tr
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard/${id}/LogBook/Summary/${driver.id}`
                        )
                      }
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-2">
                          <button
                            onClick={(e) =>
                              handleDVIRClick(e, driver.id)
                            }
                            className="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 text-xs rounded font-medium"
                          >
                            DVIR
                          </button>
                          ...
                          <button
                            onClick={(e) =>
                              handleLogsClick(e, driver.id)
                            }
                            className="bg-yellow-500 hover:bg-yellow-600 text-white px-2 py-1 text-xs rounded font-medium"
                          >
                            Logs
                          </button>
                          ...
                          <button
                            onClick={(e) =>
                              handleBackupLogsClick(e, driver.id)
                            }
                          ...
                          <button
                            ...
                            className="bg-gray-300 hover:bg-gray-400 text-black px-2 py-1 text-xs rounded font-medium"
                          >
                            DOT File
                          </button>
                        </div>
